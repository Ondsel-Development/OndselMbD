...
namespace MbD {
	...
	template<typename T>
	inline FMatDsptr EulerParametersDot<T>::pomegapE()
	{
		return EulerParameters<T>::pBpEtimesColumn(this)->times(2.0);
	}

	template<typename T>
	inline FMatDsptr EulerParametersDot<T>::pomegapEdot()
	{
		return aB()->times(2.0);
	}

	template<typename T>
	inline FColDsptr EulerParametersDot<T>::omega()
	{
		auto aaa = aB();
